...
def log_error(error_type, location, description):
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS Errors (Timestamp TEXT, Type TEXT, Location TEXT, Describe TEXT)")
    timestamp_str = datetime.now().isoformat()
    cursor.execute("INSERT INTO Errors (Timestamp, Type, Location, Describe) VALUES (?, ?, ?, ?)", (timestamp_str, error_type, location, description))
    conn.commit()
    conn.close()
